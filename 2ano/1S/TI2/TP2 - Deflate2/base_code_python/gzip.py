...
class GZIP:
    ...
    # tirado de: 3.2.2. Use of Huffman coding in the "deflate" format
    def createHuffmanFromLens(self, lenArray, verbose=True):
        ...
        # binary length count starts with max_len values initialized as zero
        bl_count = [0 for i in range(max_len+1)]
		# Get array with number of codes with length N (bl_count)
        # adds counter of 1 to max_len values of the array to the bl_count
        for N in range(1, max_len+1):
            bl_count[N] += lenArray.count(N)
        
        # Get first code of each code length (CLEN)
        code = 0
