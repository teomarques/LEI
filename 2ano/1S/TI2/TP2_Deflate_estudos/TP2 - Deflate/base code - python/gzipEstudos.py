...
class GZIP:
    ...
    def storeCLENLengths(clen_lengths):
        # Step 1: Count the number of codes for each length
        max_length = max(clen_lengths)
        bl_count = [0] * (max_length + 1)
        for length in clen_lengths:
            bl_count[length] += 1

        # Step 2: Store the lengths
        stored_lengths = []
        for length in clen_lengths:
            stored_lengths.append(length)

        return stored_lengths
    
    def createHuffmanFromLens(code_lengths):
        # Step 1: Count the number of codes for each length
        ...
        # Step 2: Determine the starting code for each length
        code = 0
        ...
        for i in range(len(code_lengths)):
            ...
            if length != 0:
                huffman_codes[i] = next_code[length]
                next_code[length] += 1
