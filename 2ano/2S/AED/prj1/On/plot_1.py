...
def compile_cpp_program(source_filename="On.cpp", output_filename="On"):
    """Compila o programa C++."""
    try:
        subprocess.run(["g++", source_filename, "-o", output_filename], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Erro de compilação: {e}")
        return False
    return True

def run_cpp_program(executable_path="./On"):
...
def main():
    ...
    for size in array_sizes:
        ...
        if execution_time is not None and found_missing is not None:
            ...
            # Verificar se o número faltante foi encontrado corretamente
    ...
    # Convertendo para numpy arrays para facilitar os cálculos
